...
#define malloc opus_heap_malloc
#define free opus_heap_free
...
/* If we've got a VS. system game, load in the palette, as well */
 void ppu_buildpalette(ppu_t *src_ppu, rgb_t *pal);
...
static void rom_checkforpal(rominfo_t *rominfo)
{
   ...
   // ppu_t *src_ppu = nes_getcontextptr()->ppu;
   // printf("address: %d.\n",(int)real_nes);
   // ppu_buildpalette(real_nes->ppu, vs_pal);

   // vid_setpalette(vs_pal);
